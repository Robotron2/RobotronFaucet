  ...
  return (
    <Card className="flex flex-col h-full bg-[#18191d] border-slate-800/60" hoverEffect={false}>
      ...
      <div className="flex flex-col gap-6 mb-8 mt-auto">
        <div className="flex items-center justify-between px-2">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">CLAIM AMOUNT</span>
            <span className="text-sm font-bold text-[#e1e1e7]">100 RBNT</span>
          </div>
          <div className="flex flex-col text-right">
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">COOLDOWN</span>
            <span className={`text-sm font-bold tracking-wider ${isOnCooldown ? "text-amber-500" : "text-[#00E676]"}`}>
      ...
      <div className="flex flex-col gap-4">
        ...
        {status === 'success' && <Alert message="Success: 100 RBNT have been added to your vault. Next claim available in 24 hours." variant="success" className="text-xs py-2 bg-transparent border-none" />}
        {status === 'error' && <Alert message="Claim failed. Please try again." variant="error" className="text-xs py-2 bg-transparent border-none" />}
      </div>
    </Card>
  );
};
